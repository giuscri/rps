...
trait GameController {
  ...
  @query
  def result(userId: Int): Future[Either[Throwable, PlayResponse]]
}
...
class GameControllerImpl(gameService: GameService)(implicit ec: ExecutionContext)
  ...
  override def result(userId: Int): Future[Either[Throwable, PlayResponse]] =
    Future {
      gameService.getResult(userId).map { play =>
